...
export default function Companion() {
  ...
  const loadThreads = useCallback(async () => {
    try {
      const r = await base44.functions.invoke("companion", { action: "threads" });
      setTierAvailable(!!r.data?.quality_tier_available);
  ...
  async function send(text) {
    ...
    try {
      const res = await fetch("/api/companion/stream", {
        ...
        body: JSON.stringify({ message: msg, tier: tierAvailable ? tier : "default", thread_id: threadId }),
      });
        ...
        <div className={`bg-card rounded-xl border border-border flex flex-col ${chatSpan}`} style={{ height: "70vh" }}>
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {busy && !messages[messages.length - 1]?.content && (
              <div className="flex justify-start"><div className="bg-muted rounded-2xl px-4 py-2.5 text-sm text-muted-foreground inline-flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> {searching ? "searching trusted sources…" : grounding ? "grounding in your evidence…" : tierAvailable && tier === "quality" ? "thinking deeply… (slower model, worth the wait)" : "thinking…"}</div></div>
            )}
            <div ref={endRef} />
          </div>
