...
export default function OuraScoresChart({ data }) {
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold">Daily Scores</h3>
          ...
          {METRICS.map((m) => (
            <button
              ...
              className={`text-[10px] px-2 py-1 rounded-full font-medium transition-colors ${
                activeMetrics.includes(m.key) ? "text-white" : "bg-muted text-muted-foreground"
              }`}
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            ...
            {METRICS.filter((m) => activeMetrics.includes(m.key)).map((m) => (
              <Line key={m.key} type="monotone" dataKey={m.key} stroke={m.color} strokeWidth={2} dot={false} name={m.label} connectNulls />
            ))}
          </LineChart>
        </ResponsiveContainer>
      </div>
