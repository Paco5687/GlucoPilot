...
| `MedicalRecord` | Owner; `filename`, `stored_as`, `content_hash`, status/upload fields, extracted title/type/date/summary/page/lab counts, `partial`, `error`, `extracted_by?`. | SHA-256 rejects byte-identical upload. Random stored filename. Mutable processing state; deletion removes file and child labs. | Failed/partial is record-level only; no page/field confidence or source location. |
| `LabResult` | Owner; `record_id`, `test_name`, `value`, `unit`, ranges?, `flag`, `collected_date`, `category`. | Unenforced parent ID. Extraction dedups `(name,date,rounded value)` within one document; reprocess replaces all children. | Missing name/value dropped; ranges may be null; no extractor version, confidence, or verification state. |
