...
| Insulin response | Insulin, nearby glucose, carbs, prior/later boluses, cycle, optional activity/position/fingerstick and daily wearable context | Builds versioned source-linked events; invalid and under-covered events are explicitly excluded, confounders remain visible, and only clean events enter time/cycle/activity/position strata. Shared quality/confidence gates values supplied to Companion/report reasoning. |
| Activity/position response | Immutable manual intervals, short timestamped wearable step intervals, glucose, clean insulin-response windows, and fixed fingerstick pairs | Manual intervals override overlapping inference without deleting it. Glucose slope, morning slope, bolus response, and CGM/fingerstick differences are stratified with samples, interval missingness, shared confidence, replication status, and noncausal language. Only qualifying effects reach Companion. |
| Management effort | Treatments, fingersticks, glucose-low context, documented device changes, overnight management events, manual activity-for-control, and append-only manual/correction rows | Visible component weights produce a measured-effort index, active minutes, and interactions. Missing source families reduce confidence rather than becoming zero. Dashboard, Visit Report, and Evidence Bundle keep outcomes separate and noncausal. |
| Clinician/specialist brief | Bounded Evidence Bundle plus governed hypothesis ledger | Specialty domain query and fail-closed entity allowlist omit unrelated PHI. Strength language, contradiction/limitation blocks, tentative hypothesis labels, and source-linked appendix remain deterministic and provider-readable. |
| Share-safe export | Explicit mode/entity/field policy, clinician brief, or bounded Analytics/Wearables Evidence Bundle | Owner/provider authorization precedes deterministic minimization. Preview time/checksum bind the download; research removes exact dates and bundle/source identities; recursive defense removes secrets, URLs, and direct/internal identifiers. |
