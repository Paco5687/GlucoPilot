...
Pump-reported and calculated TDD remain separate. A pump day is complete only when it has a usable pump-reported total or a calculated total backed by complete delivered-basal coverage. Programmed basal is not treated as delivery. Cloud-reported daily totals are additionally checked against a floor integrated from that day's scheduled-basal segments (with a 10% allowance for suspends); a total below what the programmed schedule alone would deliver is physically impossible, so it is flagged `suspect_undercount`, deselected from averages and trends, and surfaced through the pump-TDD contradiction panel until the source backfills correct data. Nutrition coverage describes logging behavior only; a day without a log does not mean zero carbohydrate intake.
